...
public class PNCInteractuable : PNCVariablesContainer
{
    ...
    public void InitializeInteractions(List<InteractionsAttemp> attemps)
    {
    ...
    private void Start()
    {
        ...
        {
            InitializeInteractions(verbs[i].attemps);
        }
        for (int i = 0; i < inventoryActions.Count; i++)
        {
            InitializeInteractions(inventoryActions[i].attemps);
        }
    }
